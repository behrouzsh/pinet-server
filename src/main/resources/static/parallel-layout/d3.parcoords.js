...
var pc = function(selection) {
  ...
  // canvas data layers
  ["shadows", "marks", "foreground", "background", "highlight"].forEach(function(layer) {
...
var side_effects = d3.dispatch.apply(this,d3.keys(__))
  ...
  .on("composite", function(d) { ctx.backround.globalCompositeOperation = d.value; })
  .on("alpha", function(d) { ctx.background.globalAlpha = d.value; })
  ...
  .on("dimensions", function(d) {
    xxscale.domain(__.dimensions);
    if (flags.interactive){pc.render().updateAxes();}
  })
  .on("bundleDimension", function(d) {
	  if (!__.dimensions.length) pc.detectDimensions();
	  if (!(__.dimensions[0] in yyscale)) pc.autoscale();
...
pc.autoscale = function() {
  ...
  __.dimensions.forEach(function(k) {
    yyscale[k] = defaultScales[__.types[k]](k);
  });
  ...
  pc.dimensions(pc.dimensions().filter(function(p,i) {
    var uniques = yyscale[p].domain().length;
  ...
  ctx.background.strokeStyle = __.color_back;
...
pc.scale = function(d, domain) {
	yyscale[d].domain(domain);

	return this;
};

pc.flip = function(d) {
	//yyscale[d].domain().reverse();					// does not work
	yyscale[d].domain(yyscale[d].domain().reverse()); // works
...
pc.commonScale = function(global, type) {
	...
	if (global) {
		var extent = d3.extent(scales.map(function(p,i) {
				return yyscale[p].domain();
			}).reduce(function(a,b) {
				return a.concat(b);
			}));

		scales.forEach(function(d) {
			yyscale[d].domain(extent);
		});

	} else {
		scales.forEach(function(k) {
			yyscale[k].domain(d3.extent(__.data, function(d) { return +d[k]; }));
		});
	}
...
pc.render['default'] = function() {
  ...
  if (__.brushed) {
    ...
    __.data.forEach(path_background);
    __.highlighted.forEach(path_highlight);
  }
};
...
function compute_cluster_centroids(d) {
	...
	__.data.forEach(function(row) {
		__.dimensions.map(function(p, i) {
			...
			value += yyscale[p](row[p]) / clusterCounts.get(scaled);
...
function compute_centroids(row) {
	...
	for (var i = 0; i < cols; ++i) {
		...
		var x = position(p[i]);
		var y = yyscale[p[i]](row[p[i]]);
		centroids.push($V([x, y]));

		// centroids on 'virtual' axes
		if (i < cols - 1) {
			var cx = x + a * (position(p[i+1]) - x);
			var cy = y + a * (yyscale[p[i+1]](row[p[i+1]]) - y);
			if (__.bundleDimension !== null) {
				var leftCentroid = __.clusterCentroids.get(yyscale[__.bundleDimension](row[__.bundleDimension])).get(p[i]);
				var rightCentroid = __.clusterCentroids.get(yyscale[__.bundleDimension](row[__.bundleDimension])).get(p[i+1]);
...
pc.axisDots = function() {
	...
	__.data.forEach(function(d) {
		__.dimensions.map(function(p, i) {
			ctx.fillRect(position(p) - 0.75, yyscale[p](d[p]) - 0.75, 1.5, 1.5);
		});
	});
	return this;
};
...
function color_path_back(d, i, ctx) {
	ctx.strokeStyle = d3.functor("#d4d5df")(d, i);
	ctx.beginPath();
	if (__.bundleDimension === null || (__.bundlingStrength === 0 && __.smoothness == 0)) {
		single_path(d, ctx);
	} else {
...
function single_path(d, ctx) {
	__.dimensions.map(function(p, i) {
		if (i == 0) {
			ctx.moveTo(position(p), yyscale[p](d[p]));
		} else {
			ctx.lineTo(position(p), yyscale[p](d[p]));
		}
	});
}
...
function path_background(d, i) {
	return color_path_back(d, i, ctx.background);
};
...
function flipAxisAndUpdatePCP(dimension, i) {
  ...
  d3.select(g[0][i])
    .transition()
      .duration(1100)
      .call(axis.scale(yyscale[dimension]));
...
pc.createAxes = function() {
  ...
  g = pc.svg.selectAll(".dimension")
    ...
    .enter().append("svg:g")
      ...
      .attr("transform", function(d) { return "translate(" + xxscale(d) + ")"; });

  // Add an axis and title.
  g.append("svg:g")
      .attr("class", "axis")
      .attr("transform", "translate(0,0)")
      .each(function(d) { d3.select(this).call(axis.scale(yyscale[d])); })
    .append("svg:text")
      .attr({
        "text-anchor": "middle",
        "y": 0,
...
pc.updateAxes = function() {
  ...
  g_data.select(".axis")
    .transition()
      .duration(1100)
      .each(function(d) {
        d3.select(this).call(axis.scale(yyscale[d]));
      });
  g_data.select(".label")
    .transition()
      .duration(1100)
  ...
  g.style("cursor", "move")
    .call(d3.behavior.drag()
      .on("dragstart", function(d) {
        ddragging[d] = this.__origin__ = xxscale(d);
        dimsAtDragstart = __.dimensions.slice();
      })
      .on("drag", function(d) {
        ddragging[d] = Math.min(w(), Math.max(0, this.__origin__ += d3.event.dx));
      ...
      .on("dragend", function(d) {
        ...
        delete ddragging[d];
        d3.select(this).transition().attr("transform", "translate(" + xxscale(d) + ")");
        pc.render();
        if (flags.shadows) paths(__.data, ctx.shadows);
      }));
  ...
  function selected() {
    ...
    var within = {
      ...
      "string": function(d,p,dimension) {
        return extents[dimension][0] <= yyscale[p](d[p]) && yyscale[p](d[p]) <= extents[dimension][1]
      }
    };
    ...
    return __.data
      .filter(function(d) {
        ...
        case "AND":
            console.log(actives.every(function(p, dimension) {
        ...
        case "OR":
            console.log(actives.some(function(p, dimension) {
                return within[__.types[p]](d,p,dimension);
            }));
    ...
    return function() {
      var p = d3.mouse(strumRect[0][0]),
          ...
          strum = {
            p1: p,
            dims: dims,
            minX: xxscale(dims.left),
            maxX: xxscale(dims.right),
            minY: 0,
            maxY: h()
          };
